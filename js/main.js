...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadAndDisplayData() { const mode=viewMode.value,rawDate=dateInput.value;if(!rawDate){jsonOutput.innerHTML='<p class="error">Vyberte prosím dátum alebo mesiac.</p>';dashboard.style.display="none";return}const[year,month]=rawDate.split("-"),fileName=`${year}_${month}.json`,filePath=`data/${fileName}`,displayPeriod="day"===mode?rawDate:`${year}-${month}`;selectedPeriodSpan.textContent=displayPeriod,jsonOutput.innerHTML="<p>Načítavam dáta...</p>",dashboard.style.display="none";try{let data=loadedDataCache[fileName];if(!data){const response=await fetch(filePath);if(!response.ok){jsonOutput.innerHTML=`<p class="error">Chyba: Dáta pre ${month}/${year} neboli nájdené.</p>`;return}data=await response.json(),loadedDataCache[fileName]=data}const aggregationResult="day"===mode?aggregateHourlyData(data,rawDate):aggregateDailyData(data,displayPeriod);if(!aggregationResult||0===aggregationResult.data.length){jsonOutput.innerHTML=`<p>Pre zvolené obdobie ${displayPeriod} neboli nájdené žiadne záznamy.</p>`;return}displaySummaryData(aggregationResult.summary,mode),displayDataInTable(aggregationResult.data,aggregationResult.mode)}catch(error){console.error("Nastala chyba pri spracovaní dát:",error),jsonOutput.innerHTML=`<p class="error">Vyskytla sa kritická chyba: ${error.message}</p>`}}
    
    function displaySummaryData(summary, mode) {
        if (!summary) { dashboard.style.display = 'none'; return; }
        
        // =======================================================
        // ZMENA: Odstránené zátvorky okolo formátovaného času
        // =======================================================
