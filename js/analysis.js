...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function displaySummary(summaries, variables) { /* ... (bez zmeny) ... */ let html="";variables.forEach(variable=>{const summary=summaries[variable],config=VARIABLES[variable],f=(val,dec=1)=>null!==val?val.toFixed(dec):"-",t=ts=>ts?`(${formatTimestampToLocalDate(ts)})`:"";html+="rain"===variable?`<div class="summary-box"><h4>Štatistiky Zrážok</h4><div class="data-points"><div class="data-point"><span>Celkový úhrn</span><span class="value">${f(summary.total,1)} mm</span></div><div class="data-point data-point-max"><span>Najvyšší denný úhrn</span><span class="value">${f(summary.max,1)} mm</span><span class="timestamp">${t(summary.maxTime)}</span></div><div class="data-point data-point-avg"><span>Priemerný denný úhrn</span><span class="value">${f(summary.avg,1)} mm</span></div></div></div>`:`<div class="summary-box"><h4>Štatistiky (${config.label})</h4><div class="data-points"><div class="data-point data-point-max"><span>Maximum</span><span class="value">${f(summary.max)} ${config.unit}</span><span class="timestamp">${t(summary.maxTime)}</span></div><div class="data-point data-point-avg"><span>Priemer</span><span class="value">${f(summary.avg)} ${config.unit}</span></div><div class="data-point data-point-min"><span>Minimum</span><span class="value">${f(summary.min)} ${config.unit}</span><span class="timestamp">${t(summary.minTime)}</span></div></div></div>`}),summaryContainer.innerHTML=html}
