...
/**
 * NOVÁ POMOCNÁ FUNKCIA: Presne vypočíta súčet prírastkov zrážok.
 * Spracuje aj vynulovanie počítadla na stanici.
...
function calculateRainIncrementSum(items) {
    ...
    // Vytvoríme si kópiu a zoradíme podľa času, aby sme zaistili správne poradie
    ...
    for (let i = 1; i < sortedItems.length; i++) {
        ...
        // Ak je súčasná hodnota menšia ako predchádzajúca, znamená to reset počítadla.
        // V takom prípade pripočítame len súčasnú hodnotu (úhrn od resetu).
...
/**
 * UPRAVENÁ FUNKCIA: Určí najčastejší smer vetra.
 * Započítava len vietor s rýchlosťou >= 1.0 m/s.
 * @param {Array} items Pole dátových záznamov.
 * @returns {number|null} Prevažujúci smer vetra v stupňoch.
 */
function calculateWindMode(items) {
    const wdCounts = new Map();
    const MIN_WIND_SPEED_FILTER = 1.0; // ZVÝŠENÁ HODNOTA
...
function calculateOverallSummary(items) {
    ...
    items.forEach(item => { const t = item.t; ['temp', 'hum', 'press'].forEach(key => { if (item[key] !== null) { metrics[key].sum += item[key]; metrics[key].count++; if (item[key] > metrics[key].max) { metrics[key].max = item[key]; metrics[key].maxT = t; } if (item[key] < metrics[key].min) { metrics[key].min = item[key]; metrics[key].minT = t; } } }); if (item.ws !== null) { metrics.ws.sum += item.ws; metrics.ws.count++; if (item.ws > metrics.ws.max) { metrics.ws.max = item.ws; metrics.ws.maxT = t; } } if (item.wg !== null) { if (item.wg > metrics.wg.max) { metrics.wg.max = item.wg; metrics.wg.maxT = t; } if (item.wg > 0 && item.wg < metrics.wg.min) { metrics.wg.min = item.wg; metrics.wg.minT = t; } } if (item.rain !== null) { metrics.rain.min = Math.min(metrics.rain.min, item.rain); metrics.rain.max = Math.max(metrics.rain.max, item.rain); } if (item.sr !== null) { metrics.sr.sum += item.sr; metrics.sr.count++; if (item.sr > metrics.sr.max) { metrics.sr.max = item.sr; metrics.sr.maxT = t; } } if (item.uv !== null) { metrics.uv.sum += item.uv; metrics.uv.count++; if (item.uv > metrics.uv.max) { metrics.uv.max = item.uv; metrics.uv.maxT = t; } } });
    
    // UPRAVENÝ VÝPOČET: Použije sa nová presná funkcia
    const rainTotal = calculateRainIncrementSum(items);

    return { tempAvg: metrics.temp.count > 0 ? metrics.temp.sum / metrics.temp.count : null, tempMax: metrics.temp.max !== -Infinity ? metrics.temp.max : null, tempMaxTime: metrics.temp.maxT, tempMin: metrics.temp.min !== Infinity ? metrics.temp.min : null, tempMinTime: metrics.temp.minT, humAvg: metrics.hum.count > 0 ? metrics.hum.sum / metrics.hum.count : null, humMax: metrics.hum.max !== -Infinity ? metrics.hum.max : null, humMaxTime: metrics.hum.maxT, humMin: metrics.hum.min !== Infinity ? metrics.hum.min : null, humMinTime: metrics.hum.minT, pressAvg: metrics.press.count > 0 ? metrics.press.sum / metrics.press.count : null, pressMax: metrics.press.max !== -Infinity ? metrics.press.max : null, pressMaxTime: metrics.press.maxT, pressMin: metrics.press.min !== Infinity ? metrics.press.min : null, pressMinTime: metrics.press.minT, wsAvg: metrics.ws.count > 0 ? metrics.ws.sum / metrics.ws.count : null, wsMax: metrics.ws.max !== -Infinity ? metrics.ws.max : null, wsMaxTime: metrics.ws.maxT, wgMax: metrics.wg.max !== -Infinity ? metrics.wg.max : null, wgMaxTime: metrics.wg.maxT, wgMin: metrics.wg.min !== Infinity ? metrics.wg.min : null, wgMinTime: metrics.wg.minT, rainTotal: rainTotal, srAvg: metrics.sr.count > 0 ? metrics.sr.sum / metrics.sr.count : null, srMax: metrics.sr.max !== -Infinity ? metrics.sr.max : null, srMaxTime: metrics.sr.maxT, uvAvg: metrics.uv.count > 0 ? metrics.uv.sum / metrics.uv.count : null, uvMax: metrics.uv.max !== -Infinity ? metrics.uv.max : null, uvMaxTime: metrics.uv.maxT, wdMode: calculateWindMode(items) };
}
...
export function aggregateHourlyData(data, rawDate) {
    ...
    summary.rainSumOfTotals = summary.rainTotal;
    summary.maxDailyRain = summary.rainTotal; // Pre denný pohľad je max. denný úhrn rovný celkovému.
    summary.maxDailyRainTime = filteredData.length > 0 ? filteredData[0].t : null;
