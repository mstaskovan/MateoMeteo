...
function calculateWindMode(items) { const wdCounts=new Map,MIN_WIND_SPEED_FILTER=.5;items.forEach(item=>{item.ws!==null&&item.ws>=MIN_WIND_SPEED_FILTER&&item.wd!==null&&wdCounts.set(item.wd,(wdCounts.get(item.wd)||0)+1)});let modeWD=null,maxCount=0;return wdCounts.forEach((count,wd)=>{count>maxCount&&(maxCount=count,modeWD=wd)}),modeWD}
