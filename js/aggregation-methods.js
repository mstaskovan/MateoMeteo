...
/**
 * ZMENA: Funkcia teraz počíta aj priemer pre SR a UV.
 */
function calculateOverallSummary(items) {
    ...
    items.forEach(item => {
        ...
        // Doplnenie výpočtov pre priemer SR a UV
        if (item.sr !== null) {
    ...
    return {
        ...
        srAvg: metrics.sr.count > 0 ? metrics.sr.sum / metrics.sr.count : null, // NOVÉ
        srMax: metrics.sr.max !== -Infinity ? metrics.sr.max : null, srMaxTime: metrics.sr.maxT,
        uvAvg: metrics.uv.count > 0 ? metrics.uv.sum / metrics.uv.count : null, // NOVÉ
...
export function aggregateHourlyData(data, rawDate) {
    ...
    let totalRainSum = 0; // NOVÁ Premenná pre súčet úhrnov
    ...
    Array.from(hourlyDataMap.keys()).sort((a, b) => a - b).forEach(hourKey => {
        ...
        if (hourSummary.rainTotal !== null) {
            totalRainSum += hourSummary.rainTotal; // Pripočítanie hodinového úhrnu
        }
        aggregatedData.push({
            ...
            rain: hourSummary.rainTotal, sr: hourSummary.srAvg, uv: hourSummary.uvAvg, // ZMENA: v tabuľke bude priemer
        });
    });

    summary.rainSumOfTotals = totalRainSum; // Pridanie súčtu do finálneho objektu
...
export function aggregateDailyData(data, selectedMonth) {
    ...
    Array.from(dailyDataMap.keys()).sort().forEach(dayKey => {
        ...
        if (daySummary.rainTotal !== null) {
            totalRainSum += daySummary.rainTotal; // Pripočítanie denného úhrnu
        }
        aggregatedData.push({
