...
export function aggregateCustomRange(data, variables, rangeInDays) {
    ...
    if (variables.includes('ws') || variables.includes('wg')) { /* ... bez zmeny ... */ }

    const summaries = {};
    variables.forEach(variable => {
        const allItems = data.filter(item => item[variable] !== null);
        if (allItems.length === 0) { summaries[variable] = { max: null, min: null, avg: null, total: null }; return; }

        if (variable === 'rain') {
            const periodTotals = aggregatedPeriods.map(p => p.values.rain?.sum).filter(v => v !== null && v !== undefined);
            const allRainRates = data.map(item => item.rr).filter(v => v !== null); // Zbierame intenzity
            ...
            if (periodTotals.length > 0) {
                ...
                summaries[variable] = {
                    ...
                    // NOVÉ: Maximálna intenzita
                    maxRate: allRainRates.length > 0 ? Math.max(...allRainRates) : null,
                };
            }
        } else {
            // ... (logika pre ostatné premenné zostáva bez zmeny)
        }
    });
